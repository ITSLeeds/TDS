...
```{r loop_bad, eval=TRUE, echo=FALSE, fig.show='hide'}
...
```{r loop_good, eval=TRUE, echo=FALSE, fig.show='hide'}
...
Write a function that can play FizzBuzz for any number of rounds, and write the resutls to the console using `message`.
...
fizzbuzz2 <- function(n){
  z = purrr::map_chr(1:n,function(x, out = c("Fizz","Buzz",x)){
    mod3 = (x %% 3) == 0
    mod5 = (x %% 5) == 0
    if(mod3){
      if(mod5){
