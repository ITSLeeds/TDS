...
**Exercise** Subset the and overwrite the `desire_lines` data frame with the `=` assignment operator so that it only has the following columns: geo_code1, geo_code2, all, bicycle, foot, car_driver, and geometry.
...
# pre stplanr::route() way of doing it, to be deleted
...
We can plot those routes using the `tmap` package, the result should appear as follows (if you set `tmap_mode("view")`)
