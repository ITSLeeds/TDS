...
Load data representing MSOA zones in West Yorkshire. 
You could download zones from a number of places, including https://www.ukdataservice.ac.uk/get-data/geography.aspx

An easy way to get zone data for Leeds is with the `pct` package, which can be installed as follows:

The zones for West Yorkshire can be shown as follows.
...
- Create a subset of the object `d` called `d_wy_origins` that only contains routes that originate in West Yorkshire
...
- Create a subset that contains only od pairs with origins and destinations in West Yorkshire

**Bonus: Convert the origin-destination data you have of West Yorkshire into desire lines**, e.g. with:

```{r, eval=FALSE}
desire_lines = stplanr::od2line(flow = d_wy_origins, zones)
