...
title: Transport and Geograpic Data Science with R
subtitle: Practical Exercises
author: Robin Lovelace and Malcolm Morgan, Institute for Transport Studies, University of Leeds
---

This practicals aim to test your knowledge of material covered in a short course delivered for the Department for Transport.
Code and data supporting the content can be found in the GitHub repository (repo) at github.com/ITSLeeds/TDS.
To see the course contents and timings see https://git.io/tds2day which links to the file `2day.md` in the repo.

```{r, engine='bash', eval=FALSE, echo=FALSE}
curl -i https://git.io -F "url=https://github.com/ITSLeeds/TDS/blob/master/courses/2day.md" -F "code=tds2day"
```

It will use the following CRAN packages:

```{r, message=FALSE, warning=FALSE, eval=FALSE}
library(dplyr)    # a data analysis package, part of the tidyverse
...
The course assumes that you have already got some basic knowledge of working with R and the editor (IDE) RStudio.
If you don't see the prerequisites at [git.io/tds2day](https://git.io/tds2day).
The main components of RStudio are shown in the figure below.
...
In some cases the the package will provide a message, as illustrated below.
...
sel = nz$Population > 100000  # create a subsetting object
nz_large <- nz[sel, ]         # subset areas with a popualtion over 100,000
canterbury <- nz[nz$Name == "Canterbury",] # subset based on 'equality' query
a_in_name <- nz[grepl(pattern = "a", x = nz$Name), ] # subset based on text string match
nz_first_and_third_column = nz[c(1, 3)]
nz_just_population = nz["Population"]
```

## ggplot2

`ggplot2` is a generic plotting package.
...
1. Create an RStudio project with an appropriate name for this course (e.g. `tds`)
1. Create a script called `set-up.R`, e.g. with the following command: `file.edit("set-up.R")`
1. Practice subsetting techniques you have learned on the `sf data.frame` object `nz`:
   1. Create an object called `nz_small` which contains only regions with less than 100,000 people
   1. Create a selection object called `sel_high_income` which is `TRUE` for regions with above median income and `FALSE` otherwise
   1. How many regions have the letter 'b' in them? What percentage of the regions in New Zealand is this?
   1. Create an object called `nz_income` which contains only the median income attribute from `nz`
   1. Bonus: plot the result to show where the high income regions are.
   1. Bonus: use `filter()` from the `dplyr` package to subset small and high income regions
...
What makes an `sf data.frame` different to a normal `data.frame` is the inclusion of a geometry column and spatial operations.
...
```{r vectorplots, out.width="40%", fig.show='hold'}
...
Lets load the locations of some mountains in New Zealand and then find which ones are within the `nz_large` areas.
...
Create plots of New Zealand's population and GDP using `plot()` and `tm_shape() + tm_polygons()` functions.

```{r, fig.show='hold', out.width="49%"}
plot(nz[c("Population", "Median_income")])
tm_shape(nz) + 
  tm_polygons()
```
...
1. Create a facetted plot showing the temporal distribution of crashes in West Yorkshire (you can choose whether to show these over the months of the year, over days of the week, or over the hours of a day)
1. Do a spatio-temporal subset to identify the crashes that happened within the most commonly traveled desire line between 07:00 and 10:00 during weekdays.
...
1. Find the route along the most traveled desire line in Leeds and plot the result.
