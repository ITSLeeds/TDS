...
Notice the use of `::` to say that you want the `od_data_smample` object from the `stplanr` package.
...
We can use the generic `plot` function to view the relationships between variaibles
...
1. Calculate the percentage who cycle in each OD pair in which at least 1 person cycles
1. Is there a positive relationship between walking and cycling in the data?
1. Plot the zones representing the `geo_code` variables in the OD data
1. Bonus: use the function `od2line()` in to convert the OD dataset into geographic desire lines
...
Load the zones for Bristol, and create a new column called `local_authority`. Make a simple map of which Local Authority each zone is in.
...
WOrk though Sections 13.1 to 13.4 of the Transport Chapter in [Geocomputation with R](https://r.geocompx.org/transport.html) 
...
```{r, echo=FASLE, eval=FALSE, include=FALSE}
# transform
