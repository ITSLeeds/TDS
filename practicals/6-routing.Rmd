...
6. Find the driving routes for `desire_top` and call them `routes_top` using `otp_plan`
...
First we need to download the roads network from the OpenStreetMap using `osmextract`. We will removed most of the paths and other features and just focus on the main roads. Then use `dodgr::weight_streetnet` to produce a graph of the road network.
...
We can convert a `dodgr` graph back into a sf data frame for plotting using `dodgr_to_sf`
...
20. Use `dodgr_contract_graph` before calculating centrality, how does this affect the computation time and the results?
