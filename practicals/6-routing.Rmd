...
nohup /usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -Xmx10000M -d64 -jar "otp_TDS/otp-1.5.0-shaded.jar" --router west-yorkshire --graphs "otp_TDS/graphs" --server --port 8080 --securePort 8081 &
```

```{r, eval=FALSE, echo=FALSE}
path_otp = "otp_TDS/otp-1.5.0-shaded.jar"
path_data = "otp_TDS"
list.files(file.path(path_data, "graphs", "west-yorkshire"))
# Download pbf file with osmextract
osmextract::oe_get("West Yorkshire")
list.files(tempdir())
# pbf_file
pbf_file = list.files(tempdir(), pattern = "osm.pbf", full.names = TRUE)
file.copy(pbf_file, file.path(path_data, "graphs", "west-yorkshire", "osm.pbf"), overwrite = TRUE)
log1 = otp_build_graph(otp = path_otp, dir = path_data, otp_version = 1.5, router = "west-yorkshire")
...
if (!file.exists("isochrone.Rds")) download.file(u, "isochrone.Rds")
```
