...
Useful `dplyr` functions are demontrated below.
...
1. Practice subsetting techniques you have learned on the `sf data.frame` object `iow`:
     ...
     1. How many regions have the number '1' in them? What percentage of the regions in the Isle of Wight is this?
     ...
     1. Bonus: use `filter()` from the `dplyr` package to subset small and high car use regions
1. Bonus: What is the population density of each region (use the 'all' column)?
...
```{r, eval=FALSE}
st_crs(iow) # 2193 the CRS for New Zealand Transverse Mercator
iow_latlng = st_transform(iow, 4326) # Transfrom from one CRS to another
st_crs(iow) # 4326 the CRS for World Geodetic System 1984
iow_latlng = st_transform(iow, 2193) # Transfrom back
```
...
Lets load the locations of some mountains in New Zealand and then find which ones are within the `iow_large` areas.
...
1. Filter crashes that happened in Isle of Wight
...
1. Create an object representing desire lines in Isle of Wight, e.g, with: `desire_lines_all = pct::get_pct_lines(region = "west-yorkshire")`
...
1. Create a facetted plot showing the temporal distribution of crashes in Isle of Wight (you can choose whether to show these over the months of the year, over days of the week, or over the hours of a day)
...
1. Get cycle route data for Isle of Wight and use the function `overline2()` to identify the routes along which most people walk to work.
